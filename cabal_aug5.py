...
def dialog_present(source: Image.Image | Path | str | None = None) -> bool:
    ...
    Trusting dialog_kind alone to say "nothing is open" turned one bad row into
    two dead cycles on 2026-08-04. The abort path asked it, got None while the
    diagnostic one line earlier had read 'extension', closed nothing, and left
    a modal covering the table. Escape could then not close the Trade window,
    and every read for the rest of that cycle and the whole of the next
    returned no rows.
...
def listing_family(rows: list[Row], name: str,
    ...
    """Live rows sharing a listing's name and price -- what a collect counts.

    Module level so the tests can drive the real thing rather than a
    reimplementation that drifts from it. Filtered STRICTLY by price, unlike
    locate_row: there an empty pool means "lost the row", here it means every
    stack at that price is gone, which is the correct answer.
    """
...
def choose_price(
    ...
    if suggested <= 0:
        ...
        # price, and parking it high beats guessing low. On a RELIST there IS
        # one, and reaching for the fallback throws away the best information
        # available.
        #
        ...
        #
        # The absolute floor still binds over it, and MIN_PLAUSIBLE_PRICE keeps
        # a misread previous price from becoming the new one.
    ...
    try:
        ...
        if shot is None:
            ...
            if dialog_kind(probe) == "extension":
                ...
                # single recheck rescuing NONE of them.
...
def _relist_cycle(row, inv_row, inv_col, dry_run, timeout, verbose, attempts, say,
    ...
    for attempt in range(1, attempts + 1):
        ...
        if target.action == "receive":
            ...
            # gone. The collect had worked every time.
            ...
            if not lost and not gained:
                ...
                # where the collect had gone through and only the client's
                # copy of the table was stale.
    ...
    for position, (index, ref, action) in enumerate(targets, start=1):
        ...
        if outcome != RELISTED:
            ...
            if not dry_run and left and require_empty_work_tab(verbose=False):
                ...
                # continued because the work tab was clean, then failed the
                # very next row on "the listings could not be read".
                #
                # close_any_dialog clicks CANCEL, never Confirmation, so it
                # cannot commit the cancellation the abort was unsure about.
                if dialog_present():
...
def leave_shop(verbose: bool = True) -> bool:
    ...
    that stops with the shop open leaves the character parked in a UI, and an
    open Trade window is exactly what makes a later find_npc fail, because it
    covers the NPC.
...
def prepare_for_actions(verbose: bool = True) -> bool:
    ...
    for attempt in range(ESCAPE_ATTEMPTS):
        # dialog_present, not dialog_kind: the other place a flaked title read
        # let a modal survive into the next cycle, where it covered the table
        # and produced "No listings visible" before a row had been touched.
        if not dialog_present():
            break
